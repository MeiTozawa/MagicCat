...
namespace dxe {
	...
	Shared<FontText> FontText::CreateAnimation(const Shared<FontTextResouce>& resource) {
		Shared<FontText> text = std::shared_ptr<FontText>(new FontText());
		text->resource_ = resource;
		text->anim_seek_ = tnl::SeekUnit::Create((float)GetFixFps(), 0, 3.0f, tnl::SeekUnit::ePlayMode::SINGLE, tnl::SeekUnit::ePlayDir::FORWARD);
		return text;
	}
	...
	Shared<FontText> FontText::createClone(const Shared<FontText>& origine) {
		...
		if (origine->anim_seek_) {
			...
			int32_t length = (int32_t)base_text_.length();			
	...
	//--------------------------------------------------------------------------------------------------------------------------
	void FontText::setString(const std::wstring& base_text) {
		{
	...
	void FontText::updateAnimation() {
		...
		//	}
		//}
		// std::wstring cutstr = base_text_.substr(0, cutpos);
		
		// to unicode
		std::wstring cutstr = base_text_.substr(0, current);

		if (anim_draw_text_ == cutstr) return;

		std::wstring addstr = cutstr.substr(anim_draw_text_.length(), -1);
