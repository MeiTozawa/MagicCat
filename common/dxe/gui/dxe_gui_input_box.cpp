...
namespace dxe {
    ...
    GuiInputBox::GuiInputBox(eInputType type
        , const std::function<void(const std::wstring& input_string)>& input_function
    ...
    //-------------------------------------------------------------------------------------------------------------------
    void GuiInputBox::setBoxString(const std::wstring& str) {
        input_string_ = str;
    ...
    void GuiInputBox::draw() {
        ...
        DrawFormatString(tx + box_width_ + 5, ty + 2, label_color, L"%s", label_.c_str());
        DrawKeyInputString(tx + 5, ty + 2, input_hdl_);
        DrawFormatString(tx + 5, ty + 2, color, L"%s", input_string_.c_str());
    }
