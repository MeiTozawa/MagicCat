...
namespace dxe {
	...
	//--------------------------------------------------------------------------------------------------------------------------
	Shared<dxe::ResourceHandle> SpriteResouce::LoadGraph(const std::wstring& file_path) {
	...
	//--------------------------------------------------------------------------------------------------------------------------
	Shared<SpriteResouce> SpriteResouce::Create(const std::wstring& file_path) {
	...
	//--------------------------------------------------------------------------------------------------------------------------
	void SpriteResouce::addDivisionProcess(const std::wstring& file_path, int32_t all_num, int32_t x_num, int32_t y_num, int32_t x_size, int32_t y_size) {
	...
	//--------------------------------------------------------------------------------------------------------------------------
	void SpriteResouce::addDivisionProcess(const std::wstring& file_path, int32_t x_size, int32_t y_size, const std::vector<int32_t>& load_frame_indexs) {
	...
	Shared<Sprite> Sprite::CreateAnimation( const Shared<SpriteResouce>& resources ) {
		...
		for (int i = 0; i < anim_num; ++i) {
			...
			ctrl->addSeekUnit( tnl::SeekUnit::Create((float)GetFixFps(), frame_num, total_time, play_mode, play_dir) );
		}
	...
	void Sprite::draw(const Shared<dxe::Camera>& camera) {
		...
		if (!is_debug_pivot_draw_) return;
		DrawCircle((int)draw_pos.x, (int)draw_pos.y, debug_pivot_radius_, debug_line_color_, true);
	...
	//--------------------------------------------------------------------------------------------------------------------------
	void Sprite::setDebugLine(bool is_draw_line, int32_t color, bool is_draw_pivot, int32_t pivot_radius) {
		is_debug_line_draw_ = is_draw_line;
		is_debug_pivot_draw_ = is_draw_pivot;
		debug_line_color_ = color;
		debug_pivot_radius_ = pivot_radius;
	}
