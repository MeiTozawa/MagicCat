...
#define DXE_PROTOTYPE_INITIALIZE							void Initialize(HINSTANCE hinstance, HWND hwnd, HDC hdc, uint32_t window_w, uint32_t window_h, int32_t fix_fps, int32_t app_start_frame) ;
#define DXE_PROTOTYPE_RELEASE								void Release() ;
#define DXE_PROTOTYPE_UPDATE								void Update( float delta_time ) ;

#define DXE_PROTOTYPE_GET_WINDOW_WIDTH						int32_t GetWindowWidth(float scale = 1.0f);
#define DXE_PROTOTYPE_GET_WINDOW_WIDTH_F					float GetWindowWidthF(float scale = 1.0f);
#define DXE_PROTOTYPE_GET_WINDOW_HEIGHT						int32_t GetWindowHeight(float scale = 1.0f);
#define DXE_PROTOTYPE_GET_WINDOW_HEIGHT_F					float GetWindowHeightF(float scale = 1.0f);
#define DXE_PROTOTYPE_GET_FIX_FPS							int32_t GetFixFps() ;
#define DXE_PROTOTYPE_GET_APP_START_FRAME					int32_t GetAppStartFrame() ;
...
#define DXE_PROTOTYPE_DRAW_STRING_F32						void DrawString(const tnl::Vector2f& pos, int color, eRectOrigin e_location, const TCHAR* str, ...) ;
...
#define DXE_PROTOTYPE_DRAW_STACK_STRING						void DrawStackString(int color, const TCHAR* str, ...) ;
...
#define DXE_PROTOTYPE_DRAW_HEXAGON							void DrawHexagon(const tnl::Vector2f& pos, float width, int color = -1, float rot = 0 ) ;
...
#define DXE_PROTOTYPE_SAVE_WORLD_ORDINARY_SETTING			void SaveWorldOrdinarySetting( const std::wstring& path ) ;
#define DXE_PROTOTYPE_LOAD_WORLD_ORDINARY_SETTING			void LoadWorldOrdinarySetting( const std::wstring& path ) ;
#define DXE_PROTOTYPE_DRAW_IMGUI_WORLD_ORDINARY_SETTING		void DrawImGuiWorldOrdinarySetting(const tnl::Vector2i& position);
...
#define DXE_PROTOTYPE_GET_DRAG_FILE_PATH_TRIGGER			std::wstring GetDragFilePathReadTrigger();
#define DXE_PROTOTYPE_GET_OPEN_READ_FILE_PATH				std::wstring GetOpenReadFilePath(const std::wstring& candidate_ext, const std::wstring& open_path = L"") ;
#define DXE_PROTOTYPE_GET_OPEN_SAVE_FILE_PATH				std::wstring GetOpenSaveFilePath(const std::wstring& ext, const std::wstring& defaultName = L"", const std::wstring& open_path = L"") ;

#define DXE_PROTOTYPE_IS_ENABLE_FILE_PATH					bool IsEnableFilePath(const TCHAR* file_name) ;
#define DXE_PROTOTYPE_IS_ENABLE_FONT_PATH					bool IsEnableFontPath(const TCHAR* font_name) ;

#define DXE_PROTOTYPE_GET_PATH_FROM_FILE_NAME				const TCHAR* GetPathFromFileName(const TCHAR* file_name) ;
#define DXE_PROTOTYPE_GET_FONT_PATH_FROM_NAME				const TCHAR* GetPathFromFontName(const TCHAR* font_name) ;
...
#define DXE_PROTOTYPE_SET_FILE_PATH_DICTIONARY				void SetFilePathDictionary(const std::unordered_map<std::wstring, const TCHAR*>& dictionary) ;
#define DXE_PROTOTYPE_SET_FONT_NAME_TO_PATH_DICTIONARY		void SetFontNameToPathDictionary(const std::unordered_map<std::wstring, const TCHAR*>& dictionary) ;
#define DXE_PROTOTYPE_SET_DRAG_FILE_PATH					void SetDragFilePath(const std::wstring& drag_file_path);
