...
namespace dxe {
	...
	public :
		...
		GuiValueSlider(
			...
			, U min
			, U max
			, const std::wstring& label = L"unknown"
		...
		void draw() {
			...
			if constexpr (GuiValueSliderTraits<int>::Value == GuiValueSliderTraits<U>::Value) {
				DrawFormatString(pos_.x + slider_lenght_ + TAB_SIZE_W2 + 5, pos_.y - TAB_SIZE_H2 + 3, color, L"%d", accessor_.get());
			}
			if constexpr (GuiValueSliderTraits<float>::Value == GuiValueSliderTraits<U>::Value) {
				DrawFormatString(pos_.x + slider_lenght_ + TAB_SIZE_W2 + 5, pos_.y - TAB_SIZE_H2 + 3, color, L"%.3f", accessor_.get());
			}
			if (is_description_ && description_function_) color = -1;
			DrawFormatString(pos_.x + slider_lenght_ + num_space, pos_.y - TAB_SIZE_H2 + 3, color, L"%s", label_.c_str());
		}
	...
	public:
		...
		void draw() {
			...
			for (int i = 0; i < 2; ++i) {
				...
				DrawString((float)pos_.x + (float)slider_lenght_ + TAB_SIZE_W2 + 5, (float)pos.y - TAB_SIZE_H2 + 3, color, L"%d", param[i] );
				DrawString((float)pos_.x + (float)slider_lenght_ + num_space, (float)pos.y - TAB_SIZE_H2 + 3, color, L"%s", ( label_ + spram[i] ).c_str() );
			}
		}
	...
	public:
		...
		void draw() {
			...
			std::wstring spram[3] = { L".x", L".y", L".z"};
			for (int i = 0; i < 3; ++i) {
				int color = (is_forcus_[i] || is_select_[i]) ? -1 : 0x88888888;
				tnl::Vector2i pos = pos_;
				pos.y += i * TAB_SIZE_H2 * 2;
				...
				DrawLine(pos_.x + slider_lenght_, pos.y - TAB_SIZE_H2, pos_.x + slider_lenght_, pos.y + TAB_SIZE_H2, color);
				DrawBox(tab_pos_[i].x - TAB_SIZE_W2, tab_pos_[i].y - TAB_SIZE_H2, tab_pos_[i].x + TAB_SIZE_W2, pos.y + TAB_SIZE_H2, color, false);
				DrawFormatString(pos_.x + slider_lenght_ + TAB_SIZE_W2 + 5, pos.y - TAB_SIZE_H2 + 3, color, L"%.3f", param[i]);
				if (is_description_ && description_function_) color = -1;
				DrawFormatString(pos_.x + slider_lenght_ + num_space, pos.y - TAB_SIZE_H2 + 3, color, L"%s", (label_ + spram[i]).c_str());
			}
		}
