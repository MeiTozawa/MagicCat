...
namespace dxe {
	...
	Shared<Mesh> Mesh::CreateFromHeightMap(unsigned char* pixels, const int t_width, const int t_height, const int t_bpp, const float width, const float depth, const float height_max, const int div_w, const int div_h) {
		...
		mesh->desc_ = std::make_shared<MeshDescHeightMap>(L"", tnl::Vector3(width, height_max, depth), div_w, div_h);
	...
	//----------------------------------------------------------------------------------------
	Shared<Mesh> Mesh::CreateFromHeightMap(const std::wstring& file_path, const float width, const float depth, const float height_max, const int div_w, const int div_h)
	{
		...
		pixels = stbi_load(&utf8_filename[0], &t_width, &t_height, &t_bpp, 0);
	...
	Shared<Mesh> Mesh::CreateFromHeightMapMV(const std::wstring& file_path, const float width, const float depth, const float height_max, const int div_w, const int div_h) {
