...
namespace dxe {
	...
	//------------------------------------------------------------------------------------------------------------------------------------
	std::shared_ptr<Texture> Texture::CreateFromMemory(char* file_data, uint32_t data_size, const std::wstring& regist_key_, bool with_srv, bool have_col_buff) {

		size_t hash = (regist_key_.empty()) ? std::hash<char*>()(file_data) : std::hash<std::wstring>()(regist_key_);
	...
	//------------------------------------------------------------------------------------------------------------------------------------
	std::shared_ptr<Texture> Texture::CreateFromColorBufferRGBA8(const uint8_t* color_buffer_rgba8, uint32_t width, uint32_t height, const std::wstring& regist_key, bool with_srv, bool have_col_buff) {

		size_t hash = 0;
		if (!regist_key.empty()) {
			hash = std::hash<std::wstring>()(regist_key);
			auto it = managed_map_.find(hash);
			if (it != managed_map_.end()) {
				return it->second;
			}
	...
	//------------------------------------------------------------------------------------------------------------------------------------
	std::shared_ptr<Texture> Texture::CreateFromFile(const std::wstring& file_path, bool with_srv, bool have_col_buff, bool is_regist_storage )
	{
		size_t hash = std::hash<std::wstring>()(file_path);
		if (is_regist_storage) {
			auto it = managed_map_.find(hash);
			if (it != managed_map_.end()) {
				return it->second;
			}
		...
		tex->file_name_ = std::get<1>(t) + L"." + std::get<2>(t);
	...
	//------------------------------------------------------------------------------------------------------------------------------------
	std::shared_ptr<Texture> Texture::CreateFromFileRegion(const std::wstring& file_path, uint32_t start_pixel_x, uint32_t start_pixel_y, uint32_t end_pixel_x, uint32_t end_pixel_y, bool with_srv, bool have_col_buff) {
		...
		std::shared_ptr<Texture> tex = CreateFromColorBufferRGBA8((unsigned char*)col_buff, pixel_w, pixel_h, file_path + std::to_wstring(start_pixel_x) + L"," + std::to_wstring(start_pixel_y) + L"," + std::to_wstring(end_pixel_x) + L"," + std::to_wstring(end_pixel_y), with_srv, have_col_buff);
	...
	//------------------------------------------------------------------------------------------------------------------------------------
	std::vector<std::shared_ptr<Texture>> Texture::CreateFromFileDiv(const std::wstring& file_path, uint32_t frame_w, uint32_t frame_h, bool with_srv, bool have_col_buff) {
		...
		for (uint32_t i = 0; i < frame_h; ++i) {
			...
			for (uint32_t k = 0; k < frame_w; ++k) {
				...
				std::shared_ptr<Texture> tex = CreateFromColorBufferRGBA8((unsigned char*)col_buff, fr_pc_w, fr_pc_h, file_path + std::to_wstring((i * frame_w) + k), with_srv, have_col_buff);
				out[( i * frame_w ) + k] = tex;
				delete[] col_buff;
			}
		}
	...
	//------------------------------------------------------------------------------------------------------------------------------------
	std::vector<std::shared_ptr<Texture>> Texture::CreateFromFileDivPixel(const std::wstring& file_path, uint32_t pixel_w, uint32_t pixel_h, bool with_srv, bool have_col_buff) {
		std::vector<std::shared_ptr<Texture>> out;
	...
	std::vector<std::shared_ptr<Texture>> Texture::CreateFromColorBufferDiv(const uint8_t* color_buffer_rgba8, uint32_t width, uint32_t height, uint32_t frame_w_num, uint32_t frame_h_num, bool with_srv, bool have_col_buff) {
		...
		for (uint32_t i = 0; i < frame_h_num; ++i) {
			...
			for (uint32_t k = 0; k < frame_w_num; ++k) {
				...
				std::shared_ptr<Texture> tex = CreateFromColorBufferRGBA8((unsigned char*)col_buff, fr_pc_w, fr_pc_h, L"", with_srv, have_col_buff);
	...
	std::shared_ptr<Texture> Texture::createShiftPixelTexture(const tnl::Vector2i& shift_pixel, bool width_srv, bool have_col_buff) {
		...
		std::shared_ptr<Texture> tex = CreateFromColorBufferRGBA8(buff, pxw, pxh, L"", width_srv, have_col_buff);
		delete[] buff;
		return tex ;
	}
	...
	std::shared_ptr<Texture> Texture::createTrimmingTexture(const tnl::Vector2i& start_position, const tnl::Vector2i& trimming_pixel_size, bool width_srv, bool have_col_buff) {
		...
		std::shared_ptr<Texture> tex = CreateFromColorBufferRGBA8(buff, trimming_pixel_size.x, trimming_pixel_size.y, L"", width_srv, have_col_buff);
		delete[] buff;
		return tex;
	}
	...
	void Texture::drawShaderResouceView(const tnl::Vector2i& pos, const float scale) {
		...
		tnl::Matrix world = tnl::Matrix::AffineTransformation({(float)pos.x - GetWindowWidthF() / 2, (float)-pos.y + GetWindowHeightF() / 2, 0}, {scale, scale, 1}, tnl::Quaternion());

		tnl::Matrix view = tnl::Matrix::LookAtLH({ 0, 0, -10 }, { 0, 0, 0 }, {0, 1, 0});
		tnl::Matrix proj = tnl::Matrix::OrthoLH(GetWindowWidthF(), GetWindowHeightF(), 1, 1000);
