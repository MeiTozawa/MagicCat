...
namespace dxe {
	...
	public :
		...
		// 淵強調
		enum class eEdgeEmphasis{ NONE, ADD, SUB };
		...
		//---------------------------------------------------------------------------------------
		// リソースから作成
		Particle(const std::wstring& resouce_file_path);
		...
		TNL_PROPERTY(float, EdgeEmphasisFactor, edge_emphasis_factor_);
		...
		ePlayType getPlayType() { return play_type_; }
		tnl::Vector2f getSize() { return {size_x_, size_y_}; }
		void setSize(const tnl::Vector2f& size) { size_x_ = size.x; size_y_ = size.y; }
	private :
		...
		void createShader();
		void sysRender(const Shared<Camera>& camera, const bool is_advance_time = true);
		void saveStatus(const std::wstring& file_path);
		void loadStatus(const std::wstring& file_path);
		...
		void setPlayType(ePlayType play_type) {
			if (play_type_ == play_type) return;
		...
		TNL_PROPERTY(eEdgeEmphasis, EdgeEmphasis, edge_emphasis_);
		...
		// 縁の強調係数 ( シェーダーでは使用していない param )  
		float edge_emphasis_factor_ = 5.0f;
		...
		// 淵の強調描画モード
		eEdgeEmphasis edge_emphasis_ = eEdgeEmphasis::NONE;
		...
		std::wstring description_str_;
		std::wstring save_path_;
		std::wstring load_path_;
