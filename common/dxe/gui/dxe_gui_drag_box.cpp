...
namespace dxe {
    ...
    GuiDragBox::GuiDragBox(const std::function<void(const std::wstring& drag_data_path)>& draged_function)
    ...
    void GuiDragBox::update() {
        ...
        std::wstring file_path = GetDragFilePathReadTrigger();
    ...
    void GuiDragBox::draw() {
        ...
        DrawFormatString(pos_.x + 5, pos_.y + 7, color, L"%s", label_.c_str());
    }
