...
#define DXE_PTCL_SHADER_COMPLIE 0
    ...
    Particle::Particle
        ...
        random_value_ = tnl::GetRandomDistribution<float>(0.0f, 1.0f);
    ...
    //--------------------------------------------------------------------------------------------------------------------------------
    Particle::Particle(const std::wstring& resouce_file_path) {
...
//
    ...
    void Particle::refreshProcess() {
        ...
        for (int i = 0; i < particle_num_; i++)
        {
            ...
            vertexs_[i].rand_value_ = tnl::GetRandomDistribution<float>(0.0f, 1.0f);
        };
	...
	void Particle::render( const Shared<Camera>& camera, const bool is_advance_time) {
        ...
        auto blend = getBlendMode();
        auto emissive = getEmissive();
        auto time_scale = getTimeScale();

        if (eEdgeEmphasis::NONE == edge_emphasis_) {
            sysRender(camera, is_advance_time);
        }
        else if (eEdgeEmphasis::ADD == edge_emphasis_) {
            setTimeScale(time_scale*0.5f);
            setBlendMode(dxe::eBlendState::SUB);
            setEmissive({ edge_emphasis_factor_, edge_emphasis_factor_, edge_emphasis_factor_ });
            sysRender(camera, is_advance_time);
            setBlendMode(dxe::eBlendState::ADD);
    ...
    //---------------------------------------------------------------------------------------------------------------------------------------------
    void Particle::sysRender(const Shared<Camera>& camera, const bool is_advance_time) {
        ...
        //
        {
            ...
            if (is_advance_time) {
                ...
                cbuffer->delta_time_ = ( 1.0f / 60.0f ) * time_scale_;
            }
            else {
    ...
    //---------------------------------------------------------------------------------------------------------------------------------------------
    void Particle::saveStatus(const std::wstring& file_path) {
        ...
        if (fp) {
            ...
            fwrite(&edge_emphasis_factor_, sizeof(float), 1, fp);
            ...
            int edge_emphasis = static_cast<int>(edge_emphasis_);
            fwrite(&edge_emphasis, sizeof(int), 1, fp);

            char filename[256] = { 0 };
            sprintf_s(filename, "%ws", texture_->getFileName().c_str());
    ...
    //---------------------------------------------------------------------------------------------------------------------------------------------
    void Particle::loadStatus(const std::wstring& file_path) {
        FILE* fp = nullptr;
        ...
        if (fp) {
            ...
            fread(&edge_emphasis_factor_, sizeof(float), 1, fp);
            ...
            int edge_emphasis = static_cast<int>(edge_emphasis_);
            fread(&edge_emphasis, sizeof(int), 1, fp);
            edge_emphasis_ = static_cast<eEdgeEmphasis>(edge_emphasis);
            ...
            texture_ = dxe::Texture::CreateFromMemory(texture_data, data_size, L"", true);
            delete[] texture_data;
