...
#define DXE_SPRITE_LOCK_GUARD_MUTEX()
	...
	public :
		SpriteDesc() = default;
		SpriteDesc(const std::wstring& file_path) : image_file_path_(file_path){}
	...
	public:
		...
		// デバッグ用ラインを描画するかどうか
		void setDebugLine( bool is_draw_line, int32_t color = -1, bool is_draw_pivot = true, int32_t pivot_radius = 2 );
	...
	private :
		...
		int32_t					debug_pivot_radius_ = 2;
		bool					is_debug_line_draw_ = false;
		bool					is_debug_pivot_draw_ = false;
	...
	public:
		...
		static Shared<dxe::ResourceHandle> LoadGraph(const std::wstring& file_path);
		...
		static Shared<SpriteResouce> Create(const std::wstring& file_path);
	...
	private:
		...
		void addDivisionProcess(const std::wstring& file_path, int32_t frame_num, int32_t x_num, int32_t y_num, int32_t x_size, int32_t y_size);
		void addDivisionProcess(const std::wstring& file_path, int32_t x_size, int32_t y_size, const std::vector<int32_t>& load_frame_indexs );
