...
namespace dxe {
	...
	Shared<Mesh> Mesh::createClone() {
		...
		if(anim_ctroller_){
			clone_mesh->anim_ctroller_ = anim_ctroller_->createClone();
		}
		return clone_mesh;
	}
	...
	//----------------------------------------------------------------------------------------
	bool Mesh::exportForFileFormatX(const std::wstring& file_path) {

		std::string save_str = getXFormatString() ;

		FILE* fp = nullptr;
		if (_wfopen_s(&fp, file_path.c_str(), L"w") == 0) {
	...
	//----------------------------------------------------------------------------------------
	bool Mesh::exportForFileFormatMV(const std::wstring& file_path) {
		if (mesh_format_ == eMeshFormat::MESH_FMT_PG) {
			Shared<Mesh> mesh;
			exportForFileFormatX(L"temp.x");
			mesh = CreateFromFileMV(L"temp.x");
			DeleteFileW(L"temp.x");
	...
	//----------------------------------------------------------------------------------------
	bool Mesh::ExportForFileFormatMVT(const std::wstring& file_path, std::vector<Mesh*>& mesh_tbl) {

		for (uint32_t i = 0; i < mesh_tbl.size(); ++i) {
			MV1SaveModelToMV1File(mesh_tbl[i]->mv_hdl_, ( file_path + std::to_wstring(i)).c_str());
		}

		std::unordered_map<std::wstring, Shared<dxe::Texture>> tex_map;
		for (uint32_t i = 0; i < mesh_tbl.size(); ++i) {
		...
		for (uint32_t i = 0; i < mesh_tbl.size(); ++i) {
			...
			uint32_t mesh_data_lenght = (uint32_t)tnl::GetFileSize((file_path + std::to_wstring(i)).c_str());
		...
		*((uint32_t*)p) = (uint32_t)tex_map.size(); p += sizeof(uint32_t);
		...
		for (uint32_t i = 0; i < mesh_tbl.size(); ++i) {
			...
			uint32_t mesh_data_length = (uint32_t)tnl::GetFileSize((file_path + std::to_wstring(i)).c_str());
			...
			*((uint32_t*)p) = mesh_data_length;			p += sizeof(uint32_t);
			...
			// write mesh data
			FILE* fp = nullptr;
			_wfopen_s(&fp, (file_path + std::to_wstring(i)).c_str(), L"rb");
			if ( fp ) {
		...
		for (int i = 0; i < mesh_tbl.size(); ++i) {
			DeleteFileW((file_path + std::to_wstring(i)).c_str());
		}
	...
	std::vector<Shared<Mesh>> Mesh::CreateFromFileMVT(const std::wstring& file_path) {
		...
		if (fp) {
			...
			for (uint32_t i = 0; i < all_texture_num; ++i) {
				...
				wchar_t* name = new wchar_t[ tex_name_size + 1 ];
				memset(name, 0, tex_name_size + 1);
				memcpy(name, p, tex_name_size);
				p += tex_name_size;
				dxe::Texture::CreateFromMemory(p, tex_data_size, std::wstring(name));
				p += tex_data_size;
				delete[] name;
			}
			...
			for (uint32_t i = 0; i < meshs.size(); ++i) {
				...
				if (mesh_name_size) {
					name = new wchar_t[mesh_name_size + 1];
					memset(name, 0, mesh_name_size + 1);
					memcpy(name, p, mesh_name_size);
					mesh->setName(std::wstring(name));
					p += mesh_name_size;
					delete[] name;
				}
				...
				for (uint32_t k = 0; k < tex_num; ++k) {
					...
					memset(name, 0, tex_name_size + 1);
					memcpy(name, p, tex_name_size);
					Shared<dxe::Texture> tex = dxe::Texture::CreateFromFile(std::wstring(name));
	...
	//----------------------------------------------------------------------------------------
	Shared<Mesh> Mesh::CreateFromFileMV(const std::wstring& file_path, const float base_scl)
	{
		Shared<Mesh> mesh = Shared<Mesh>(new Mesh());
		...
		DxLib::VECTOR dxv = MV1GetMeshMaxPosition(mesh->mv_hdl_, 0);
		mesh->bd_box_size_ = tnl::Vector3(dxv.x, dxv.y, dxv.z) * 2.0f * base_scl;
		mesh->bd_sphere_radius_ = mesh->bd_box_size_.length() * 0.5f;
	...
	void Mesh::render(const Shared<dxe::Camera> camera) {
		...
		bool light_enable = DxLib::GetLightEnable();
		if (light_enable) {
		...
		//mt_trans = tnl::Vector3::TransformCoord(origin_position_, origin_rotation_) 
		tnl::Vector3 v_trans = tnl::Vector3::TransformCoord(offset_position_from_origin_, offset_rotation_from_origin_ * rot_) + pos_;
		mt_trans = tnl::Matrix::Translation(v_trans.x, v_trans.y, v_trans.z);

		//mt_rotation = rot_.getMatrix();
		mt_rotation = (offset_rotation_from_origin_ * rot_).getMatrix();
		...
		if (flg_dbg_line_ & Mesh::fDebugLine::FLG_OBB)			DrawOBB(camera, pos_, bd_box_size_, rot_ );
	...
	Shared<Mesh> Mesh::CreateConvertMV(Shared<Mesh> mesh) {
		...
		//MATERIALPARAM mt;
		//mt.Ambient = GetColorF(TNL_DEP_V3(mesh->render_param_.getMtrlAmbient()), 1);
		//mt.Diffuse = GetColorF(TNL_DEP_V3(mesh->render_param_.getMtrlDiffuse()), 1);
		//mt.Emissive = GetColorF(TNL_DEP_V3(mesh->render_param_.getMtrlEmissive()), 1);
		//mt.Specular = GetColorF(TNL_DEP_V3(mesh->render_param_.getMtrlSpecular()), 1);
		//mt.Power = mesh->render_param_.getMtrlSpecPower();
		//new_mesh->mv_hdl_ = MV1CreateSimpleModel(mesh->vtxs_.data(), mesh->vtxs_.size(), mesh->idxs_.data(), mesh->idxs_.size(), &mt, 0);
	...
	//----------------------------------------------------------------------------------------
	void Mesh::setMtrlAmbient(const tnl::Vector3& col, int mv_idx) noexcept {
		render_param_.setMtrlAmbient(col);
		if (mesh_format_ != eMeshFormat::MESH_FMT_MV) return;
		int mt_num = MV1GetMaterialNum(mv_hdl_);
		if (mv_idx < 0) {
			for (int i = 0; i < mt_num; ++i) {
				MV1SetMaterialAmbColor(mv_hdl_, i, render_param_.dxlib_mtrl_.Ambient);
			}
		}
		else {
			if (mv_idx >= mt_num) return;
			MV1SetMaterialAmbColor(mv_hdl_, mv_idx, render_param_.dxlib_mtrl_.Ambient);
		}
	}

	//----------------------------------------------------------------------------------------
	void Mesh::setMtrlDiffuse(const tnl::Vector3& col, int mv_idx) noexcept {
		render_param_.setMtrlDiffuse(col);
		if (mesh_format_ != eMeshFormat::MESH_FMT_MV) return;
		int mt_num = MV1GetMaterialNum(mv_hdl_);
		if (mv_idx < 0) {
			for (int i = 0; i < mt_num; ++i) {
				MV1SetMaterialDifColor(mv_hdl_, i, render_param_.dxlib_mtrl_.Diffuse);
			}
		}
		else {
			if (mv_idx >= mt_num) return;
			MV1SetMaterialDifColor(mv_hdl_, mv_idx, render_param_.dxlib_mtrl_.Diffuse);
		}
	}

	//----------------------------------------------------------------------------------------
	void Mesh::setMtrlSpecular(const tnl::Vector3& col, int mv_idx) noexcept {
		render_param_.setMtrlSpecular(col);
		if (mesh_format_ != eMeshFormat::MESH_FMT_MV) return;
		int mt_num = MV1GetMaterialNum(mv_hdl_);
		if (mv_idx < 0) {
			for (int i = 0; i < mt_num; ++i) {
				MV1SetMaterialSpcColor(mv_hdl_, i, render_param_.dxlib_mtrl_.Specular);
			}
		}
		else {
			if (mv_idx >= mt_num) return;
			MV1SetMaterialSpcColor(mv_hdl_, mv_idx, render_param_.dxlib_mtrl_.Specular);
		}
	}

	//----------------------------------------------------------------------------------------
	void Mesh::setMtrlEmissive(const tnl::Vector3& col, int mv_idx) noexcept {
		render_param_.setMtrlEmissive(col);
		if (mesh_format_ != eMeshFormat::MESH_FMT_MV) return;
		int mt_num = MV1GetMaterialNum(mv_hdl_);
		if (mv_idx < 0) {
			for (int i = 0; i < mt_num; ++i) {
				MV1SetMaterialEmiColor(mv_hdl_, i, render_param_.dxlib_mtrl_.Emissive);
			}
		}
		else {
			if (mv_idx >= mt_num) return;
			MV1SetMaterialEmiColor(mv_hdl_, mv_idx, render_param_.dxlib_mtrl_.Emissive);
		}
	}

	//----------------------------------------------------------------------------------------
	void Mesh::setMtrlSpecPower(const float pow, int mv_idx) noexcept {
		render_param_.setMtrlSpecPower(pow);
		if (mesh_format_ != eMeshFormat::MESH_FMT_MV) return;
		int mt_num = MV1GetMaterialNum(mv_hdl_);
		if (mv_idx < 0) {
			for (int i = 0; i < mt_num; ++i) {
				MV1SetMaterialSpcPower(mv_hdl_, i, render_param_.dxlib_mtrl_.Power);
			}
		}
		else {
			if (mv_idx >= mt_num) return;
			MV1SetMaterialSpcPower(mv_hdl_, mv_idx, render_param_.dxlib_mtrl_.Power);
		}
	}

	//----------------------------------------------------------------------------------------
	void Mesh::setRenderParam(const RenderParam& param, int mv_idx) {
		render_param_ = param;
		if (mesh_format_ != eMeshFormat::MESH_FMT_MV) return;
		int mt_num = MV1GetMaterialNum(mv_hdl_);
		if (mv_idx < 0) {
	...
	//----------------------------------------------------------------------------------------
	void Mesh::addAnim(const std::wstring& file_path, tnl::SeekUnit::ePlayMode play_mode) {
		int mv1 = MV1LoadModel(file_path.c_str());
		int att = MV1AttachAnim(mv_hdl_, 1, mv1);
		float fix_fps = (float)dxe::GetFixFps() ;
		int frame = MV1GetAttachAnimTotalTime(mv_hdl_, att);
		Shared<tnl::SeekUnit> seek = tnl::SeekUnit::Create(fix_fps, frame, (frame / fix_fps), play_mode, tnl::SeekUnit::ePlayDir::FORWARD);
		Shared<MeshAnim> anim = std::make_shared<MeshAnim>() ;
		anim->anim_hdl_ = mv1;
		anim->seek_ = seek;
		if (!anim_ctroller_) {
			anim_ctroller_ = std::make_shared<MeshAnimCotroller>();
		}
		anim_ctroller_->animations_.emplace_back(anim);
		MV1DetachAnim(mv_hdl_, att);
	}

	//----------------------------------------------------------------------------------------
	void Mesh::setAnim(int32_t anim_index, bool is_play_transitioned, float change_blend_time, bool force_update) {
		if (!anim_ctroller_) return;
		if (anim_ctroller_->animations_.size() <= anim_index) return;
		if (!force_update) {
			if (anim_ctroller_->pic_anim_index_ == anim_index) {
				return ;
			}
		}
		anim_ctroller_->playing_state_ = MeshAnimCotroller::ePlayingState::TRANSITIONING;
		anim_ctroller_->is_play_transitioned_ = is_play_transitioned;
		anim_ctroller_->transitioning_blend_time_count_ = 0;
		anim_ctroller_->transitioning_blend_time_target_ = change_blend_time;
		anim_ctroller_->pic_anim_index_ = anim_index;
		anim_ctroller_->pic_anim_ = anim_ctroller_->animations_[anim_index];
		anim_ctroller_->pic_attach_index_ = MV1AttachAnim(mv_hdl_, 1, anim_ctroller_->pic_anim_->anim_hdl_);
		anim_ctroller_->attach_indexs_.emplace_back(anim_ctroller_->pic_attach_index_);
	}

	//----------------------------------------------------------------------------------------
	int Mesh::getPicupAnimIndex() {
		if (!anim_ctroller_) return -1;
		return anim_ctroller_->pic_anim_index_;
	}

	//----------------------------------------------------------------------------------------
	MeshAnimCotroller::ePlayingState Mesh::getAnimState() {
		if (!anim_ctroller_) return MeshAnimCotroller::ePlayingState::ENDED ;
		return anim_ctroller_->playing_state_ ;
	}

	//----------------------------------------------------------------------------------------
	void Mesh::playAnim() {
		if (!anim_ctroller_) return;
		if (MeshAnimCotroller::ePlayingState::TRANSITIONING == anim_ctroller_->playing_state_) {
			anim_ctroller_->is_play_transitioned_ = true;
			return ;
		}
		anim_ctroller_->playing_state_ = MeshAnimCotroller::ePlayingState::PLAYING;
		anim_ctroller_->pic_anim_->seek_->play();
	}
	//----------------------------------------------------------------------------------------
	void Mesh::restartAnim() {
		if (!anim_ctroller_) return;
		if (MeshAnimCotroller::ePlayingState::TRANSITIONING == anim_ctroller_->playing_state_) {
			anim_ctroller_->is_play_transitioned_ = true;
			return;
		...
		anim_ctroller_->playing_state_ = MeshAnimCotroller::ePlayingState::PLAYING;
		anim_ctroller_->pic_anim_->seek_->restart();
	}
	//----------------------------------------------------------------------------------------
	void Mesh::pauseAnim() {
		if (!anim_ctroller_) return;
		if (MeshAnimCotroller::ePlayingState::TRANSITIONING == anim_ctroller_->playing_state_) {
			anim_ctroller_->is_play_transitioned_ = false;
			return;
		}
		anim_ctroller_->playing_state_ = MeshAnimCotroller::ePlayingState::PAUSE;
		anim_ctroller_->pic_anim_->seek_->pause();
	}
	//----------------------------------------------------------------------------------------
	void Mesh::updateAnim(float delta_time) {

		if (MeshAnimCotroller::ePlayingState::TRANSITIONING == anim_ctroller_->playing_state_) {
			if (anim_ctroller_->transitioning_blend_time_target_ > FLT_EPSILON) {
				anim_ctroller_->transitioning_blend_time_count_ += dxe::GetDeltaTime();
				float blend_rate = anim_ctroller_->transitioning_blend_time_count_ / anim_ctroller_->transitioning_blend_time_target_ ;
				blend_rate = std::clamp(blend_rate, 0.0f, 1.0f);
				for (int i = 0; i < anim_ctroller_->attach_indexs_.size(); ++i) {
					MV1SetAttachAnimBlendRate(mv_hdl_, anim_ctroller_->attach_indexs_[i], 1.0f - blend_rate);
				}
				MV1SetAttachAnimBlendRate(mv_hdl_, anim_ctroller_->pic_attach_index_, blend_rate );
			}
			if (anim_ctroller_->transitioning_blend_time_count_ >= anim_ctroller_->transitioning_blend_time_target_) {
				anim_ctroller_->playing_state_ = (anim_ctroller_->is_play_transitioned_) ? MeshAnimCotroller::ePlayingState::PLAYING : MeshAnimCotroller::ePlayingState::STANDBY;

				if (anim_ctroller_->is_play_transitioned_ ){
					anim_ctroller_->playing_state_ = MeshAnimCotroller::ePlayingState::PLAYING;
					anim_ctroller_->pic_anim_->seek_->restart();
				}
				else {
					anim_ctroller_->playing_state_ = MeshAnimCotroller::ePlayingState::STANDBY;
				}

				for (int i = 0; i < anim_ctroller_->attach_indexs_.size(); ++i) {
					if (anim_ctroller_->attach_indexs_[i] == anim_ctroller_->pic_attach_index_) continue;
					MV1DetachAnim(mv_hdl_, anim_ctroller_->attach_indexs_[i]);
				}
				anim_ctroller_->attach_indexs_.clear();
				anim_ctroller_->attach_indexs_.emplace_back( anim_ctroller_->pic_attach_index_ );

			}
		}
		else if (MeshAnimCotroller::ePlayingState::PLAYING == anim_ctroller_->playing_state_) {
			anim_ctroller_->pic_anim_->seek_->update( delta_time );
			float rate = anim_ctroller_->pic_anim_->seek_->getSeekRate();
			float at = MV1GetAttachAnimTotalTime(mv_hdl_, anim_ctroller_->attach_indexs_[0]);
			MV1SetAttachAnimTime(mv_hdl_, anim_ctroller_->attach_indexs_[0], at * rate);

			if (anim_ctroller_->pic_anim_->seek_->getCondition() == tnl::SeekUnit::eCondition::STANDBY) {
				anim_ctroller_->playing_state_ = MeshAnimCotroller::ePlayingState::STANDBY;
			}
			else if (anim_ctroller_->pic_anim_->seek_->getCondition() == tnl::SeekUnit::eCondition::ENDED) {
				anim_ctroller_->playing_state_ = MeshAnimCotroller::ePlayingState::ENDED;
			}
