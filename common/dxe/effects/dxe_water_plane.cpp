...
#define DXE_WATER_PLANE_SHADER_COMPLIE 0
	...
	//
	//
	WaterPlane::WaterPlane(const std::wstring& file_path) {
		initialize();
	...
	//-----------------------------------------------------------------------------------------------------------------------------------------------------
	void WaterPlane::loadPatternTexture(const std::wstring& pattern_texture_path) {
		tex_pattern_ = dxe::Texture::CreateFromFile(pattern_texture_path, true);
		tex_pattern_->createShaderResourceView();
	}

	//-----------------------------------------------------------------------------------------------------------------------------------------------------
	void WaterPlane::loadDecorationTexture(const std::wstring& decoration_texture_path) {
		tex_decoration_ = dxe::Texture::CreateFromFile(decoration_texture_path, true);
	...
	void WaterPlane::setWaveDirection(const eWaveDirection& e_wave_direction) {
		if (e_wave_direction_ == e_wave_direction) return;
	...
	void WaterPlane::render(const Shared<dxe::Camera>& camera) {
		...
		if (use_landing_cut_out_tex_ && use_landing_cut_out_tex_) {
	...
	//-------------------------------------------------------------------------------------------------------------------------------
	void WaterPlane::saveStatus(const std::wstring& file_path) {
		FILE* fp = nullptr;
		...
		if (fp) {
			...
			if (use_pattern_tex_) {
				...
				swprintf_s(filename, L"%s", tex_pattern_->getFileName().c_str());
			...
			if (use_decoration_tex_) {
				wchar_t filename[256] = { 0 };
				swprintf_s(filename, L"%s", tex_decoration_->getFileName().c_str());
				fwrite(filename, sizeof(filename), 1, fp);
	...
	//-------------------------------------------------------------------------------------------------------------------------------
	void WaterPlane::loadStatus(const std::wstring& file_path) {
		...
		if (fp) {
			...
			if (use_pattern_tex_) {
				...
				tex_pattern_ = dxe::Texture::CreateFromMemory(texture_data, data_size);
				tex_pattern_->createShaderResourceView();
				delete[] texture_data;
			}
			...
			if (use_decoration_tex_) {
				...
				char* texture_data = new char[data_size];
				fread(texture_data, data_size, 1, fp);
				tex_decoration_ = dxe::Texture::CreateFromMemory(texture_data, data_size);
				tex_decoration_->createShaderResourceView();
				delete[] texture_data;
			}
