...
namespace dxe {
	...
	void Input::drawDebugJoypadInfo(const tnl::Vector2f& position) {
		if (eJoypad::UNUSED == use_joypad_id_) {
			DrawStringF(position.x, position.y, L"unused joypad", -1);
			return;
		}
		if (!checkEnable()) {
			DrawStringF(position.x, position.y, L"not enable stage", -1);
			return;
		}
		...
		y += 20; DrawFormatStringF(position.x, y, -1, L"%d%d%d%d%d", joystate.Buttons[0] >> 7, joystate.Buttons[1] >> 7, joystate.Buttons[2] >> 7, joystate.Buttons[3] >> 7, joystate.Buttons[4] >> 7);
		y += 20; DrawFormatStringF(position.x, y, -1, L"%d%d%d%d%d", joystate.Buttons[5] >> 7, joystate.Buttons[6] >> 7, joystate.Buttons[7] >> 7, joystate.Buttons[8] >> 7, joystate.Buttons[9] >> 7);
		y += 20; DrawFormatStringF(position.x, y, -1, L"%d%d%d%d%d", joystate.Buttons[10] >> 7, joystate.Buttons[11] >> 7, joystate.Buttons[12] >> 7, joystate.Buttons[13] >> 7, joystate.Buttons[14] >> 7);
		y += 20; DrawFormatStringF(position.x, y, -1, L"%d%d%d%d%d", joystate.Buttons[15] >> 7);
		y += 20; DrawFormatStringF(position.x, y, -1, L"%d", joystate.ThumbLX);
		y += 20; DrawFormatStringF(position.x, y, -1, L"%d", joystate.ThumbLY);
		y += 20; DrawFormatStringF(position.x, y, -1, L"%d", joystate.ThumbRX);
		y += 20; DrawFormatStringF(position.x, y, -1, L"%d", joystate.ThumbRY);
		y += 20; DrawFormatStringF(position.x, y, -1, L"%d", joystate.LeftTrigger);
		y += 20; DrawFormatStringF(position.x, y, -1, L"%d", joystate.RightTrigger);
	}
