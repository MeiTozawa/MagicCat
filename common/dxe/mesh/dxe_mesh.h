...
namespace dxe {
	...
	class MeshAnim final {
	public :
		~MeshAnim() {
			if (anim_hdl_) MV1DeleteModel(anim_hdl_);
		}
		Shared<MeshAnim> createClone() {
			Shared<MeshAnim> clone = std::make_shared<MeshAnim>();
			clone->anim_hdl_ = MV1CreateCloneModel(anim_hdl_);
			clone->seek_ = seek_->createClone();
			return clone;
		};
	private :
		friend class Mesh ;
	...
	public :
		...
		Shared<MeshAnimCotroller> createClone() {
			Shared<MeshAnimCotroller> clone = std::make_shared<MeshAnimCotroller>();
			clone->attach_indexs_ = attach_indexs_;
			for (auto anim : animations_) {
				clone->animations_.emplace_back( anim->createClone() );
			}
			return clone ;
		};
	private :
		friend class Mesh ;
		bool							is_play_transitioned_ = false;
		int								pic_anim_index_ = -1;
		int								pic_attach_index_ = -1;
		float							transitioning_blend_time_count_ = 0.0f;
		float							transitioning_blend_time_target_ = 0.0f;
		Shared<MeshAnim>				pic_anim_ = nullptr;
		ePlayingState					playing_state_ = ePlayingState::STANDBY;
		std::vector<int>				attach_indexs_;
		std::vector<Shared<MeshAnim>>	animations_;
	};
	...
	public :
		...
		TNL_PROPERTY(tnl::Vector3, OffsetPositionFromOrigin, offset_position_from_origin_);
		TNL_PROPERTY(tnl::Quaternion, OffsetRotationFromOrigin, offset_rotation_from_origin_);
		...
		bool exportForFileFormatX(const std::wstring& file_path);
		...
		bool exportForFileFormatMV(const std::wstring& file_path);
		...
		// アニメーションデータのファイルパス
		// 再生モード
		void addAnim(const std::wstring& file_path, tnl::SeekUnit::ePlayMode play_mode);

		// 再生するアニメーションを設定
		// arg1... アニメーションインデックス ( addAnim のコール順に 0 から )
		// arg2... 現在のアニメーションから遷移後、即再生するか
		// arg3... アニメーション遷移の時間
		// arg4... 現在再生中のアニメーションと同じものが設定された場合、再設定するか
		void setAnim(int32_t anim_index, bool is_play_transitioned, float change_blend_time = 0, bool force_update = false);

		// 現在設定中のアニメーションインデックスを取得
		int getPicupAnimIndex();

		// アニメーションの再生状態を取得
		MeshAnimCotroller::ePlayingState getAnimState();

		// 現在設定中のアニメーションを再生する( pause 状態からの途中再生 )
		void playAnim();

		// リスタート再生 ( 単発再生のアニメーションを再度再生させるなど )
		void restartAnim();
		...
		// 名前の取得
		inline void setName(const std::wstring& name) noexcept { mesh_name_ = name; }
		inline const std::wstring& getName() const noexcept { return mesh_name_; }
		...
		// アンビエントカラー設定
		void setMtrlAmbient(const tnl::Vector3& col, int mv_idx = 0) noexcept override;

		// ディフューズカラー設定
		void setMtrlDiffuse(const tnl::Vector3& col, int mv_idx = 0) noexcept override;

		// スペキュラカラー設定
		void setMtrlSpecular(const tnl::Vector3& col, int mv_idx = 0) noexcept override;

		// エミッシヴカラー設定
		void setMtrlEmissive(const tnl::Vector3& col, int mv_idx = 0) noexcept override;

		// スペキュラパワー設定
		void setMtrlSpecPower(const float pow, int mv_idx = 0) noexcept override;

		// 一括設定
		void setRenderParam(const RenderParam& param, int mv_idx = 0) override;
		...
		// ret.... [ true : 保存成功 ] [ false : 保存失敗 ]
		// tips... ロードの高速化措置として用意
		static bool ExportForFileFormatMVT(const std::wstring& file_path, std::vector<Mesh*>& mesh_tbl);
		...
		static Shared<Mesh> CreatePlaneMV(
			const tnl::Vector3& size_wh
			, const int div_w = 10
			, const int div_h = 10
			, const bool is_left_cycle = false
			, const tnl::Vector3& ltop_uv = { 0, 0, 0 }
			, const tnl::Vector3& rbottom_uv = { 1, 1, 0 } ) noexcept;
		...
		// arg5... 横分割数
		// arg6... 縦分割数
		static Shared<Mesh> CreateFromHeightMapMV(const std::wstring& file_path, const float width, const float depth, const float height_max, const int div_w = 100, const int div_h = 100);
		...
		// arg1... フィルパス
		// arg2... スケール( デフォルト1.0f )
		static std::vector<Shared<Mesh>> CreateFromFileObjMV(const std::wstring& file_path, const float base_scl = 1.0f);

		//-----------------------------------------------------------------------------------------------------
		// DXLib が対応している形式ファイルからメッシュを作成
		// arg1... フィルパス
		// arg2... スケール( デフォルト1.0f )
		static Shared<Mesh> CreateFromFileMV(const std::wstring& file_path, const float base_scl = 1.0f);
		...
		// 独自形式の MVT ファイルから生成
		// arg1... フィルパス
		static std::vector<Shared<Mesh>> CreateFromFileMVT(const std::wstring& file_path);
	...
	private:
		...
		tnl::Vector3 offset_position_from_origin_ = { 0, 0, 0 };
		tnl::Quaternion offset_rotation_from_origin_;

		Shared<MeshVbo>		vbo_  = nullptr;
		Shared<MeshDesc>	desc_ = nullptr;
		Shared<MeshAnimCotroller> anim_ctroller_ = nullptr;

		std::wstring mesh_name_;
		...
		static Shared<Mesh> CreateFromHeightMap(const std::wstring& file_path, const float width, const float depth, const float height_max, const int div_w = 100, const int div_h = 100);
		...
		static std::vector<Shared<Mesh>> CreateFromFileObj(const std::wstring& file_path, const float scl = 1.0f);
