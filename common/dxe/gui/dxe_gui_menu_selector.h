...
namespace dxe {
	...
	public:
		...
		GuiMenuSelector(const std::function<void(int32_t)>& sel_func, tMenu... tMenus) {
			const std::wstring array[] = { static_cast<std::wstring>(tMenus)... };
			for (auto elem : array) {
		...
		// メニューの追加
		void addMenu(const std::wstring& menu_str) { menus_.emplace_back(menu_str); }
