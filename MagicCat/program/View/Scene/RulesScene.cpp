...
namespace mc
{
    ...
    public:
        ...
        void Update(float deltaTime) override
        {
            ...
            renderService->DrawString(boxX1 + TEXT_START_OFFSET_X, boxY1 + TEXT_START_OFFSET_Y, L"【ルール説明】", COLOR_WHITE);

            int textY = boxY1 + CONTENT_START_OFFSET_Y;
            renderService->DrawString(boxX1 + INDENT_LEVEL_1, textY, L"仕組み: カードを引くと即座に効果が発動します", COLOR_TEXT_NORMAL);
            renderService->DrawString(boxX1 + INDENT_LEVEL_2, textY + LINE_SPACING, L"魔法カード：MPが回復しする。", COLOR_TEXT_BLUE);
            renderService->DrawString(boxX1 + INDENT_LEVEL_2, textY + LINE_SPACING * 2, L"じゃんけんカード：敵の該当する攻撃のウェイトが上昇する。", COLOR_TEXT_RED);

            renderService->DrawString(boxX1 + INDENT_LEVEL_1, textY + SECTION_SPACING, L"アクション: 【カードを引く】【攻撃する】【魔法を使用する】", COLOR_TEXT_NORMAL);
            renderService->DrawString(boxX1 + INDENT_LEVEL_2, textY + SECTION_SPACING + LINE_SPACING, L"魔法を使用するとMPが必要。効果は何か試してごらん。", COLOR_TEXT_GREEN);
            renderService->DrawString(boxX1 + INDENT_LEVEL_2, textY + SECTION_SPACING + LINE_SPACING * 2, L"攻撃した後、ターンが終了する。", COLOR_TEXT_RED);
            renderService->DrawString(boxX1 + INDENT_LEVEL_2, textY + SECTION_SPACING + LINE_SPACING * 3, L"ターンが終了すると、敵のウェイトと手札はリセットされる。", COLOR_TEXT_RED);

            int kbrHandle = assetService->GetImageHandle(EImage::KB_R);
            DrawGraph(boxX2 - ICON_OFFSET_X, boxY2 - ICON_OFFSET_Y, kbrHandle, TRUE);
            renderService->DrawString(boxX2 - ICON_TEXT_OFFSET_X, boxY2 - ICON_TEXT_OFFSET_Y, L"押して戻る", COLOR_TEXT_NORMAL);
        }
    };

    std::unique_ptr<IScene> CreateRulesScene(IInputService* inputService, ISceneService* sceneService, IAssetService* assetService, IRenderService* renderService)
    {
