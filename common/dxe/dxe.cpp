...
namespace dxe {
	...
	std::unordered_map<std::wstring, const TCHAR*> ref_file_path_dictionary ;
	std::unordered_map<std::wstring, const TCHAR*> ref_font_name_to_path_dictionary ;

	int32_t g_window_w = 0;
	int32_t g_window_h = 0;
	int32_t g_app_start_frame = 0;
	int32_t g_fix_fps = 0;

	int32_t GetWindowWidth(float scale) { 
		if (0 == g_window_w) {
			throw std::runtime_error("GetWindowWidth is Not Initialize");
		}
		return (int32_t)(g_window_w * scale) ; 
	}
	float GetWindowWidthF(float scale) { 
		if (0 == g_window_w) {
			throw std::runtime_error("GetWindowWidth is Not Initialize");
		}
		return static_cast<float>(g_window_w) * scale; 
	}

	int32_t GetWindowHeight(float scale) { 
		if (0 == g_window_h) {
			throw std::runtime_error("GetWindowHeight is Not Initialize");
		}
		return (int32_t)(g_window_h * scale) ; 
	}
	float GetWindowHeightF(float scale) { 
		if (0 == g_window_h) {
			throw std::runtime_error("GetWindowHeight is Not Initialize");
		}
		return static_cast<float>(g_window_h) * scale; 
	}

	int32_t GetFixFps() { return g_fix_fps; }
	int32_t GetAppStartFrame() { return g_app_start_frame; }

	//------------------------------------------------------------------------------------------------------------------------------
	void Initialize(HINSTANCE hinstance, HWND hwnd, HDC hdc, uint32_t window_w, uint32_t window_h, int32_t fix_fps, int32_t app_start_frame) {

		g_window_w = window_w;
		g_window_h = window_h;
		g_app_start_frame = app_start_frame;
		g_fix_fps = fix_fps;
		...
		// tnl 入力制御初期化
		tnl::Input::Initialize(hinstance, hwnd, hdc, g_window_w, g_window_h);
		...
		unsigned char* img1_buff = new unsigned char[4 * PIXEL_NUM];
		int* p = (int*)img1_buff;
		...
		unsigned char* img2_buff = new unsigned char[4 * PIXEL_NUM];
		p = (int*)img2_buff;
		*p = -1; ++p;
	...
	bool IsRunApplication() {
		if (g_application_start_frame_count < g_app_start_frame) {
	...
	//------------------------------------------------------------------------------------------------------------------------------
	void DrawString(const tnl::Vector2f& pos, int color, eRectOrigin e_location, const TCHAR* str, ...) {

		TCHAR buff[1024] = { 0 };
		va_list argptr;
		va_start(argptr, str);
		vswprintf(buff, 1024, str, argptr);
		va_end(argptr);

		int font_size = GetFontSize();
		int size_x, size_y, line_count;
		GetDrawStringSize(&size_x, &size_y, &line_count, buff, (int)wcslen(buff));
	...
	//------------------------------------------------------------------------------------------------------------------------------
	void DrawStringToHandle(const tnl::Vector2f& pos, int color, int font_hdl, eRectOrigin e_location, const TCHAR* str, ...) {
		TCHAR buff[1024] = { 0 };
		va_list argptr;
		va_start(argptr, str);
		vswprintf(buff, 1024, str, argptr);
		...
		int font_size = GetFontSize();
		int size_x, size_y, line_count;
		GetDrawStringSizeToHandle(&size_x, &size_y, &line_count, buff, (int)wcslen(buff), font_hdl);
	...
	//------------------------------------------------------------------------------------------------------------------------------
	void DrawStackString(int color, const TCHAR* str, ...) {
		TCHAR buff[1024] = { 0 };
		va_list argptr;
		va_start(argptr, str);
		vswprintf(buff, 1024, str, argptr);
		...
		if (g_statck_string_position.y + size_y >= g_window_h) {
	...
	void DrawFpsIndicator(const tnl::Vector2i& pos) {
		...
		dxe::DrawStringToHandle(tnl::Vector2f( (float)pos.x + 210.0f, (float)pos.y), -1, font_hdl, eRectOrigin::LEFT_TOP, L"%.1f / %.1f fps", g_fix_fps * fps_rate, unlim_fps);
	}
	...
	//------------------------------------------------------------------------------------------------------------------------------
	void DrawHexagon(const tnl::Vector2f& pos, float width, int color, float rot ) {
		...
		if (fabs(rot) >= FLT_EPSILON) {
			for (int i = 0; i < 6; ++i) {
				vertex[i] = tnl::Vector3::TransformCoord(vertex[i], tnl::Quaternion::RotationAxis({ 0,0,1 }, rot));
			}
		}

		//DrawCircle(pos.x, pos.y, radius, 0xffff00ff, false);
		for (int i = 0; i < 5; ++i) {
			DxLib::DrawLine((int)(pos.x + vertex[i].x), (int)(pos.y + vertex[i].y), (int)(pos.x + vertex[i + 1].x), (int)(pos.y + vertex[i + 1].y), color);
		}
		DxLib::DrawLine((int)(pos.x + vertex[5].x), (int)( pos.y + vertex[5].y ), (int)( pos.x + vertex[0].x ), (int)( pos.y + vertex[0].y ), color);
	}
	...
	void DrawDottedGrid(const tnl::Vector2f& position, const float square_size, int w_num, int h_num, const Shared<dxe::Camera>& camera) {
		...
		if (!camera) {
			...
			for (int i = 0; i < w_num + 1; ++i) {
				s.x = position.x + i * square_size;
				s.y = position.y;
				e.x = position.x + i * square_size;
				e.y = position.y + l;
				dxe::DrawDottedLine(s, e, false);
			}
			l = square_size * w_num;
			for (int i = 0; i < h_num + 1; ++i) {
				s.x = position.x;
				s.y = position.y + i * square_size;
				e.x = position.x + l;
				e.y = position.y + i * square_size;
				dxe::DrawDottedLine(s, e, false);
			}
		}
		else {
			...
			float l = square_size * h_num;
			for (int i = 0; i < w_num + 1; ++i) {
				s.x = x + position.x + i * square_size;
				s.y = y + position.y;
				e.x = x + position.x + i * square_size;
				e.y = y + position.y + l;
			...
			for (int i = 0; i < h_num + 1; ++i) {
				s.x = x + position.x;
				s.y = y + position.y + i * square_size;
				e.x = x + position.x + l;
				e.y = y + position.y + i * square_size;
	...
	//------------------------------------------------------------------------------------------------------------------------------
	std::wstring GetDragFilePathReadTrigger() {
		std::wstring path = g_drag_file_path;
		g_drag_file_path.clear();
		return path;
	}

	//------------------------------------------------------------------------------------------------------------------------------
	void SetFilePathDictionary(const std::unordered_map<std::wstring, const TCHAR*>& dictionary) {
	...
	//------------------------------------------------------------------------------------------------------------------------------
	void SetFontNameToPathDictionary(const std::unordered_map<std::wstring, const TCHAR*>& dictionary) {
		for (const auto& [key, value] : dictionary) {
	...
	//------------------------------------------------------------------------------------------------------------------------------
	bool IsEnableFilePath(const TCHAR* file_name) {
		return (ref_file_path_dictionary.end() != ref_file_path_dictionary.find(file_name) );
	}

	//------------------------------------------------------------------------------------------------------------------------------
	bool IsEnableFontPath(const TCHAR* font_name) {
		return (ref_font_name_to_path_dictionary.end() != ref_font_name_to_path_dictionary.find(font_name));
	}

	//------------------------------------------------------------------------------------------------------------------------------
	const TCHAR* GetPathFromFileName(const TCHAR* file_name) {
		auto it = ref_file_path_dictionary.find(file_name);
		if (it == ref_file_path_dictionary.end()) {
			throw std::runtime_error(std::string("dxe::GetPathFromFileName Not Found Error"));
		}
		return it->second ;
	}

	//------------------------------------------------------------------------------------------------------------------------------
	const TCHAR* GetPathFromFontName(const TCHAR* font_name) {
		auto it = ref_font_name_to_path_dictionary.find(font_name);
		if (it == ref_font_name_to_path_dictionary.end()) {
			throw std::runtime_error(std::string("dxe::GetFontPathFromName Not Found Error"));
		}
		return it->second;
	}


#define IMGUI_CHECK_BOX(label, get_func, set_func)\
{\
bool flag = get_func() ;\
...
#define IMGUI_SLIDER_FLOAT(label, get_func, set_func, vmin, vmax)\
{\
...
#define IMGUI_INPUT_FLOAT2(label, get_func, set_func)\
{\
float value[2] ;\
get_func(&value[0], &value[1]) ;\
ImGui::InputFloat2(label,value);\
set_func(value[0], value[1]) ;\
}\

#define IMGUI_INPUT_VECTOR(label, get_func, set_func)\
{\
DxLib::VECTOR v = get_func() ;\
...
#define IMGUI_COLOR_EDIT3(label, get_func, set_func)\
{\
int v[3] ;\
float fv[3] ;\
get_func(&v[0],&v[1],&v[2]);\
fv[0] = (float)v[0] / 255.0f ;\
fv[1] = (float)v[1] / 255.0f ;\
fv[2] = (float)v[2] / 255.0f ;\
ImGui::ColorEdit3(label, fv);\
v[0] = (int)( fv[0] * 255.0f );\
v[1] = (int)(fv[1] * 255.0f);\
v[2] = (int)( fv[2] * 255.0f );\
set_func(v[0],v[1],v[2]);\
}\

#define IMGUI_COLOR_F_EDIT3(label, get_func, set_func)\
{\
DxLib::COLOR_F col ;\
float fv[3] ;\
col = get_func();\
...
set_func(col);\
	...
	//------------------------------------------------------------------------------------------------------------------------------
	void SaveWorldOrdinarySetting( const std::wstring& path ) {
		int i1;
		int i3[3];
		int i4[4];
		float f1;
		...
		if (fp) {
			GetBackgroundColor(&i4[0], &i4[1], &i4[2], &i4[3]);
			fwrite(i4, sizeof(int), 4, fp);
			...
			GetFogStartEnd(&f2[0], &f2[1]);
			fwrite(f2, sizeof(float), 2, fp);

			GetFogColor(&i3[0], &i3[1], &i3[2]);
			fwrite(i3, sizeof(int), 3, fp);

			i1 = GetLightEnable();
			fwrite(&i1, sizeof(int), 1, fp);

			vec = GetLightDirection();
			fwrite(&vec.x, sizeof(float), 1, fp);
			fwrite(&vec.y, sizeof(float), 1, fp);
			fwrite(&vec.z, sizeof(float), 1, fp);

			fcol = GetLightDifColor();
			fwrite(&fcol.r, sizeof(float), 1, fp);
			fwrite(&fcol.g, sizeof(float), 1, fp);
			fwrite(&fcol.b, sizeof(float), 1, fp);
			fwrite(&fcol.a, sizeof(float), 1, fp);

			fcol = GetLightAmbColor();
			fwrite(&fcol.r, sizeof(float), 1, fp);
			fwrite(&fcol.g, sizeof(float), 1, fp);
			fwrite(&fcol.b, sizeof(float), 1, fp);
			fwrite(&fcol.a, sizeof(float), 1, fp);

			fcol = GetLightSpcColor();
			fwrite(&fcol.r, sizeof(float), 1, fp);
			fwrite(&fcol.g, sizeof(float), 1, fp);
			fwrite(&fcol.b, sizeof(float), 1, fp);
			fwrite(&fcol.a, sizeof(float), 1, fp);
	...
	//------------------------------------------------------------------------------------------------------------------------------
	void LoadWorldOrdinarySetting(const std::wstring& path) {
		int i1;
		int i3[3];
		int i4[4];
		float f1;
		...
		if (fp) {
			...
			fread(i4, sizeof(int), 4, fp);
			SetBackgroundColor(i4[0], i4[1], i4[2], i4[3]);
			...
			fread(f2, sizeof(float), 2, fp);
			SetFogStartEnd(f2[0], f2[1]);

			fread(i3, sizeof(int), 3, fp);
			SetFogColor(i3[0], i3[1], i3[2]);

			fread(&i1, sizeof(int), 1, fp);
			SetLightEnable(i1);

			fread(&vec.x, sizeof(float), 1, fp);
			fread(&vec.y, sizeof(float), 1, fp);
			fread(&vec.z, sizeof(float), 1, fp);
			SetLightDirection(vec);

			fread(&fcol.r, sizeof(float), 1, fp);
			fread(&fcol.g, sizeof(float), 1, fp);
			fread(&fcol.b, sizeof(float), 1, fp);
			fread(&fcol.a, sizeof(float), 1, fp);
			SetLightDifColor(fcol);

			fread(&fcol.r, sizeof(float), 1, fp);
			fread(&fcol.g, sizeof(float), 1, fp);
			fread(&fcol.b, sizeof(float), 1, fp);
			fread(&fcol.a, sizeof(float), 1, fp);
			SetLightAmbColor(fcol);

			fread(&fcol.r, sizeof(float), 1, fp);
			fread(&fcol.g, sizeof(float), 1, fp);
			fread(&fcol.b, sizeof(float), 1, fp);
			fread(&fcol.a, sizeof(float), 1, fp);
			SetLightSpcColor(fcol);
	...
	//------------------------------------------------------------------------------------------------------------------------------
	void DrawImGuiWorldOrdinarySetting(const tnl::Vector2i& position) {

		std::string label = "world ordinary Setting";

		//  新しいウィンドウの作成
		ImGui::StyleColorsClassic();
		ImGui::SetNextWindowPos(ImVec2((float)position.x, (float)position.y), ImGuiCond_FirstUseEver);
		ImGui::SetNextWindowSize(ImVec2(400, 400));
		ImGui::Begin(label.c_str(), nullptr, ImGuiWindowFlags_NoResize);

		if (ImGui::Button("save"))
		{
			std::wstring path = dxe::GetOpenSaveFilePath(L"wos", L"world_ordinary.wos");
			if (!path.empty()) {
				SaveWorldOrdinarySetting(path);
			}
		}
		ImGui::SameLine();
		if (ImGui::Button("load")) {
			std::wstring path = dxe::GetOpenReadFilePath(L"*.wos");
			if (!path.empty()) {
				LoadWorldOrdinarySetting(path);
			}
		}

		IMGUI_COLOR_EDIT3("BackGroundColor", GetBackgroundColor, SetBackgroundColor);

		IMGUI_CHECK_BOX("FogEnable", GetFogEnable, SetFogEnable);
		IMGUI_SLIDER_FLOAT("FogDensity", GetFogDensity, SetFogDensity, 0, 1.0f);
		IMGUI_INPUT_FLOAT2("FogDistance", GetFogStartEnd, SetFogStartEnd);
		IMGUI_COLOR_EDIT3("FogColor", GetFogColor, SetFogColor);

		IMGUI_CHECK_BOX("DefaultLightEnable", GetLightEnable, SetLightEnable);
		IMGUI_INPUT_VECTOR("LightDirection", GetLightDirection, SetLightDirection);
		IMGUI_COLOR_F_EDIT3("LightDiffColor", GetLightDifColor, SetLightDifColor);
		IMGUI_COLOR_F_EDIT3("LightAmbiColor", GetLightAmbColor, SetLightAmbColor);
		IMGUI_COLOR_F_EDIT3("LightSpecColor", GetLightSpcColor, SetLightSpcColor);
	...
	void SetDragFilePath(const std::wstring& drag_file_path) {
	...
	//------------------------------------------------------------------------------------------------------------------------------
	std::wstring GetOpenReadFilePath(const std::wstring& candidate_ext, const std::wstring& open_path) {
		wchar_t fullPath[MAX_PATH] = { 0 };
		wchar_t fileName[MAX_PATH] = { 0 };
		wchar_t openPath[MAX_PATH] = { 0 };
		...
		// --- フィルタ文字列の構築 ---
		std::wstring filterDesc = candidate_ext;
		...
		if (!open_path.empty()) {
			ofn.lpstrInitialDir = open_path.data();
		}
		else {
			auto path = std::filesystem::current_path();
			swprintf_s(openPath, MAX_PATH, path.wstring().data());
	...
	//------------------------------------------------------------------------------------------------------------------------------
	std::wstring GetOpenSaveFilePath(const std::wstring& ext, const std::wstring& defaultName, const std::wstring& open_path) {
		wchar_t fullPath[MAX_PATH] = { 0 };
		wchar_t openPath[MAX_PATH] = { 0 };
		...
		if (!defaultName.empty()) {
			...
			wcsncpy_s(fullPath, defaultName.c_str(), _TRUNCATE);
		}
		...
		// フィルタ設定（前回同様）
		std::wstring filterDesc = ext + L" File (*." + ext + L")";
		std::wstring filterPattern = L"*." + ext;
		std::vector<wchar_t> filter;
		filter.insert(filter.end(), filterDesc.begin(), filterDesc.end());
		filter.push_back('\0');
