...
namespace dxe {
	...
	public :
		...
		// ファイルパスの取得
		const std::wstring& getFilePath() { return file_path_; }

		// ファイル名の取得
		const std::wstring& getFileName() { return file_name_; }
		...
		// 生成
		// arg1... ファイルパス
		static std::shared_ptr<Texture> CreateFromFile( const std::wstring& file_path, bool with_srv = false, bool have_col_buff = false, bool is_regist_storage = false );
		static std::shared_ptr<Texture> CreateFromMemory( char* file_data, uint32_t data_size, const std::wstring& regist_key_ = L"", bool with_srv = false, bool have_col_buff = false);
		static std::shared_ptr<Texture> CreateFromColorBufferRGBA8(const uint8_t* color_buffer_rgba8, uint32_t width, uint32_t height, const std::wstring& regist_key, bool with_srv = false, bool have_col_buff = false);
		static std::shared_ptr<Texture> CreateFromDxLibGraphHdl( int dxlib_graph_hdl );
		static std::shared_ptr<Texture> CreateFromFileRegion(const std::wstring& file_path, uint32_t start_pixel_x, uint32_t start_pixel_y, uint32_t end_pixel_x, uint32_t end_pixel_y, bool width_srv = false, bool have_col_buff = false);
		static std::vector<std::shared_ptr<Texture>> CreateFromFileDiv(const std::wstring& file_path, uint32_t frame_w, uint32_t frame_h, bool width_srv = false, bool have_col_buff = false);
		static std::vector<std::shared_ptr<Texture>> CreateFromFileDivPixel(const std::wstring& file_path, uint32_t pixel_w, uint32_t pixel_h, bool width_srv = false, bool have_col_buff = false);
