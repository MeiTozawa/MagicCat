...
namespace tnl {
	...
	{
		FILE* fp = nullptr ;
		auto err = _wfopen_s(&fp, fileName, L"rb");
		if (err != 0) return -1LL;
	...
	std::tuple<std::wstring, std::wstring, std::wstring>
		DetachmentFilePath(const std::wstring& file_path) {

		uint32_t n1 = static_cast<uint32_t>(file_path.find_last_of(L"/"));
		if (UINT_MAX == n1) n1 = static_cast<uint32_t>(file_path.find_last_of(L"\\"));
		std::wstring path = file_path.substr(0, n1);

		uint32_t n2 = static_cast<uint32_t>(file_path.find_last_of(L"."));
		std::wstring file = file_path.substr(n1 + 1, n2 - path.length() - 1);

		uint32_t n3 = static_cast<uint32_t>(file_path.length()) - n2;
		std::wstring ext = file_path.substr(n2 + 1, n3);

		return std::make_tuple(path, file, ext);
	}

	//----------------------------------------------------------------------------------------------
	bool GetStringLine(std::wstring& line, std::wstring& remaining) {
		size_t nlpos = remaining.find(L"\n");
		if (nlpos == std::wstring::npos) return false;
	...
	//----------------------------------------------------------------------------------------------
	std::tuple<std::unique_ptr<uint8_t[]>, uint32_t, uint32_t, uint32_t> LoadGraphicColorBuffer(const std::wstring& file_path) {
		...
		stb_memory = stbi_load(&utf8_filename[0], &width, &height, &bpp, 0);
	...
	//----------------------------------------------------------------------------------------------
	void AddFont(const std::wstring& file_path) {
#ifdef UNICODE
		//wchar_t buff[256] = { 0 };
		//tnl::ToWChara(buff, file_path);
		DESIGNVECTOR design;
	...
	{
		if (wstrDest == NULL || strSrc == NULL || length < 1) {
			//assert(0);
		}
	...
	std::string WStringToString(const std::wstring& src)
	{
		...
		// wstring (UTF-16) → SJIS (CP_ACP) への変換サイズを確認
		int iBufferSize = WideCharToMultiByte(CP_ACP, 0, src.c_str(), -1, NULL, 0, NULL, NULL);

		// バッファの取得
		char* cpSJIS = new char[iBufferSize];

		// 実際の変換処理
		WideCharToMultiByte(CP_ACP, 0, src.c_str(), -1, cpSJIS, iBufferSize, NULL, NULL);

		// stringの生成 (終端ヌル文字を除いた範囲を指定)
		std::string oRet(cpSJIS, cpSJIS + iBufferSize - 1);
	...
	//----------------------------------------------------------------------------------------------
	std::wstring FloatToString(float value, const std::wstring& fmt) {
		wchar_t buf[64] = { 0 };
